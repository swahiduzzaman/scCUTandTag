...
run_sicer2_all_input() {
    ...
    for CELL in "${CELLTYPES[@]}"; do
        ...
        if [[ ! -f "${MARK_DIR}/${MARK}_${CELL}.bam" ]]; then
            echo "⚠️ Treatment BAM missing: ${MARK}_${CELL}.bam. Skipping..."
            continue
        fi
        if [[ ! -f "${MARK_DIR}/input_${CELL}.bam" ]]; then
            echo "⚠️ Control BAM missing: input_${CELL}.bam. Skipping..."
            continue
        fi
...
run_sicer2_no_input() {
    ...
    for CELL in "${CELLTYPES[@]}"; do
        ...
        echo "✅ Finished: ${MARK}_${CELL} (SICER2, no input)"
        echo
    done
    ...
    # Peak summary
    #------------------------------------------
    echo "========= SICER2 Peak Summary for $MARK ($MODE, No Input) ========="
    for CELL in "${CELLTYPES[@]}"; do
        DIR="${OUT_DIR}/${MARK}_${CELL}"
        PEAK_FILE=$(find "$DIR" -type f -name "*-FDR0.05-island.bed" | head -n 1)
        if [[ -f "$PEAK_FILE" ]]; then
